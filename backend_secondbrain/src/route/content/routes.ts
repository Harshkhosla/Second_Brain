...
export class ContentRoutes {
    ...
    public static Deletecontent = async (req: Request, res: Response) => {
        try {
            ...
            const updateddata = await ContentDetails.findByIdAndDelete(DeleteContent)
            console.log(updateddata);
            if(!updateddata){
                res.status(404).json({
                    message :"Your content dosent dosent exists"
                })
                return
            }

            const remainingData = await ContentDetails.find({ userId: users._id }).populate("tags");
